...
export function activate(context: vscode.ExtensionContext) {
  ...
  let saveTabsDisposable = vscode.commands.registerCommand(
    "extension.saveOpenTabs",
    () => {
      // 現在開いているエディタを取得
      ...
      vscode.workspace.textDocuments.forEach(doc => {
        if (doc.uri.scheme === "file") {
          doc.save().then(() => {
            // ドキュメントを閉じて再度開くことでキャッシュをクリア
            vscode.commands.executeCommand('workbench.action.closeActiveEditor').then(() => {
              vscode.workspace.openTextDocument(doc.uri).then(document => {
  ...
  let openTabsDisposable = vscode.commands.registerCommand(
    "extension.openSavedTabs",
    async () => {
      // .openTabs フォルダのパスを指定
      const openTabsDir = path.join(
        vscode.workspace.rootPath || "",
      ...
      if (fs.existsSync(openTabsDir)) {
        ...
        if (files.length > 0) {
          ...
          if (selected) {
            const filePath =
              selected === "最新"
                ? path.join(openTabsDir, files[0])
                : path.join(openTabsDir, selected);
            const openTabs = fs
        ...
        vscode.window.showErrorMessage(
          `Directory ${openTabsDir} does not exist.`
        );
      }
    }
  );
  ...
  let clearTabsDisposable = vscode.commands.registerCommand(
    "extension.clearSavedTabs",
    () => {
      // .openTabs フォルダのパスを指定
      const openTabsDir = path.join(
        vscode.workspace.rootPath || "",
      ...
      if (fs.existsSync(openTabsDir)) {
        ...
        vscode.window.showInformationMessage(
          `Cleared all files in ${openTabsDir}`
        );
      } else {
        vscode.window.showErrorMessage(
          `Directory ${openTabsDir} does not exist.`
